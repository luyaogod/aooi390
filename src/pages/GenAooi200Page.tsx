...
function GenAooi200Page() {
  ...
  const handleGen = async () => {
    if (!selectedConnection) { toast.error('请先选择外部服务器'); return }
  ...
  const handleImportTemplate = async () => {
    ...
    try {
      const result = await window.electronAPI.aooi200ImportTemplate(queryMode, queryMode === 'external' ? importConnectionName : undefined)
      if (result.canceled) return
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-4">
          {connectionsLoading ? (
            ...
            <Field label="外部服务器">
              <Select value={selectedConnection} onValueChange={(value) => { setSelectedConnection(value); setGenResult(null); setError(null) }}>
                <SelectTrigger className="w-72"><SelectValue placeholder="选择外部数据库连接" /></SelectTrigger>
                <SelectContent><SelectGroup>{connections.map(conn => <SelectItem key={conn.name} value={conn.name}>{conn.name}{conn.isDefault ? ' (默认)' : ''}</SelectItem>)}</SelectGroup></SelectContent>
              </Select>
            </Field>
          )}
        ...
        <CardFooter className="gap-2">
          <Button variant="outline" size="sm" onClick={handleGen} disabled={!selectedConnection || genLoading} className="gap-1.5">{genLoading ? <Loader2 className="size-3.5 animate-spin" /> : <Download className="size-3.5" />}{genLoading ? '获取中...' : '获取系统配置'}</Button>
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-4">
          <Field label="数据源: 选择从哪里查询生成模板所需的数据">
            <RadioGroup value={queryMode} onValueChange={(v) => setQueryMode(v as 'internal' | 'external')} options={[{ label: '本地默认配置', value: 'internal', description: '从已同步的本地数据库查询' }, { label: '外部数据库', value: 'external', description: '直连 Oracle/Kingbase 查询' }]} />
          </Field>
          {queryMode === 'external' && !connectionsLoading && connections.length > 0 && (
            <Field label="选择外部连接">
              <Select value={importConnectionName} onValueChange={(value) => { if (value) setImportConnectionName(value) }}>
        ...
        <CardContent className="flex flex-col gap-4">
          <Field label="选择企业">
            <div className="flex items-center gap-2">
              <Select value={selectedEnt} onValueChange={(value) => { setSelectedEnt(value); setWfPreviewRows([]); setWfError(null); setWfResult(null) }} disabled={entList.length === 0}>
                <SelectTrigger className="w-52"><SelectValue placeholder="请先加载企业列表" /></SelectTrigger>
              ...
              <Button variant="outline" size="sm" onClick={handleLoadEntList} disabled={entListLoading} className="gap-1.5">{entListLoading ? <Loader2 className="size-3.5 animate-spin" /> : <Database className="size-3.5" />}{entListLoading ? '加载中...' : '加载企业'}</Button>
            </div>
          </Field>
