...
/** ENT同步表固定配置 */
const ENT_SYNC_TABLES: EntSyncTableConfig[] = [
...
/** 单张表预览结果 */
export interface EntSyncPreview {
  tableName: string;
  entField: string;
  sourceCount: number;
}
...
/** 单张表同步结果 */
export interface EntSyncStepResult {
  tableName: string;
  success: boolean;
  sourceCount: number;
  tempCount: number;
...
/** 全量同步结果 */
export interface EntSyncAllResult {
  success: boolean;
 ...
 */
export class EntSyncService {
  /**
   * 获取 ENT 同步表配置列表
   */
  public getSyncTables(): EntSyncTableConfig[] {
    return ENT_SYNC_TABLES;
  }
  ...
  public async getEntList(): Promise<number[]> {
    ...
    try {
      ...
      logger.error(error, '[EntSyncService] 查询ENT列表失败');
      throw error;
    }
  }
   ...
   */
  public async preview(sourceEnt: number): Promise<EntSyncPreview[]> {
    await this.ensureConnected();

    const results: EntSyncPreview[] = [];

    for (const table of ENT_SYNC_TABLES) {
      try {
        ...
        logger.error(error, '[EntSyncService] 预览表 %s 失败', table.tableName);
        results.push({
          tableName: table.tableName,
          entField: table.entField,
  ...
  public async syncTable(
    ...
    sourceEnt: number,
    targetEnt: number
  ): Promise<EntSyncStepResult> {
    const result: EntSyncStepResult = {
      tableName,
      success: false,
      sourceCount: 0,
      tempCount: 0,
    ...
    try {
      ...
      logger.info('[EntSyncService] %s: 创建临时表完成', tableName);

      // 2. 更新临时表中的 ENT 字段为目标值
      await externalDB.query(
        `UPDATE "${tempTable}" SET "${entField}" = ${targetEnt}`
      );
      logger.info('[EntSyncService] %s: 更新临时表ENT字段完成', tableName);
      ...
      logger.info('[EntSyncService] %s: 删除目标ENT数据 %d 条', tableName, result.deletedCount);
      ...
      logger.info('[EntSyncService] %s: 插入数据 %d 条', tableName, result.insertedCount);
      ...
      result.success = true;
      logger.info('[EntSyncService] %s: 同步完成，验证条数 %d', tableName, result.verifyCount);
    } catch (error) {
      result.error = error instanceof Error ? error.message : String(error);
      logger.error(error, '[EntSyncService] %s: 同步失败', tableName);
    } finally {
      // 7. 清理临时表（无论成功失败都要清理）
      try {
        await externalDB.query(`DROP TABLE IF EXISTS "${tempTable}"`);
        logger.info('[EntSyncService] %s: 临时表已清理', tableName);
      } catch (dropError) {
        logger.error(dropError, '[EntSyncService] %s: 清理临时表失败', tableName);
      }
    }
   ...
   * 执行全量 ENT 同步
   */
  public async syncAll(sourceEnt: number, targetEnt: number): Promise<EntSyncAllResult> {
    await this.ensureConnected();

    if (ENT_SYNC_TABLES.length === 0) {
      return {
        success: false,
        results: [],
        message: 'ENT同步配置为空',
    ...
    logger.info('[EntSyncService] 开始ENT同步: 源=%d, 目标=%d, 共%d张表', sourceEnt, targetEnt, ENT_SYNC_TABLES.length);

    const results: EntSyncStepResult[] = [];
    let successCount = 0;
    ...
    const allSuccess = successCount === ENT_SYNC_TABLES.length;
    const message = allSuccess
      ? `ENT同步完成，${successCount}/${ENT_SYNC_TABLES.length} 张表同步成功`
      : `ENT同步部分完成，${successCount}/${ENT_SYNC_TABLES.length} 张表同步成功`;

    logger.info('[EntSyncService] %s', message);
  ...
  private async ensureConnected(): Promise<void> {
    ...
    try {
      ...
      logger.info('[EntSyncService] 外部数据库已连接: %s', defaultConn.name);
    } catch (error) {
      logger.error(error, '[EntSyncService] 连接外部数据库失败');
      throw error;
    }
  }
}
...
export const entSyncService = new EntSyncService();
